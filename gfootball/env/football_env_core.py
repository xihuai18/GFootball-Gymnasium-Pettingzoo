...
class FootballEnvCore(object):
    ...
    def _get_new_env(self):
        env = libgame.GameEnv()
        env.game_config.physics_steps_per_frame = self._config["physics_steps_per_frame"]
    ...
    def _reset(self, animations, inc):
        ...
        assert not scenario_config.dynamic_player_selection or not scenario_config.control_all_players, (
    ...
    def _rendering_in_use(self):
        ...
        if not self._use_rendering_engine:
            assert not _active_rendering, (
                "Environment does not support multiple " "rendering instances in the same process."
            )
    ...
    def step(self, action, extra_data={}):
            ...
            for i in range(agents):
                ...
                if self._env.waiting_for_game_count == 20:
                    ...
                    if (
                        ...
                        and controlled_players[i] == self._observation["ball_owned_player"]
                    ):
        ...
        if (
            ...
            and self._observation["game_mode"] != int(libgame.e_GameMode.e_GameMode_Normal)
            and self._state.previous_game_mode == int(libgame.e_GameMode.e_GameMode_Normal)
        ):
            self._env.state = GameState.game_done
        ...
        if episode_done:
            ...
            logging.info(
                "Episode reward: %.2f score: [%d, %d], steps: %d, " "FPS: %.1f, gameFPS: %.1f",
    ...
    def _retrieve_observation(self):
        ...
        if self._env.game_config.render:
            ...
            frame = np.reshape(frame, [self._config["render_resolution_x"], self._config["render_resolution_y"], 3])
            frame = np.reshape(
                np.concatenate([frame[:, :, 0], frame[:, :, 1], frame[:, :, 2]]),
                [3, self._config["render_resolution_y"], self._config["render_resolution_x"]],
            )
    ...
    def observation(self):
        ...
        assert (
            self._env.state == GameState.game_running or self._env.state == GameState.game_done
