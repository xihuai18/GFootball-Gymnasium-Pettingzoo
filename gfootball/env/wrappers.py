...
class PixelsStateWrapper(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        ...
        for observation in obs:
            ...
            frame = cv2.resize(
                frame, (self._channel_dimensions[0], self._channel_dimensions[1]), interpolation=cv2.INTER_AREA
            )
            if self._grayscale:
...
class SMMWrapper(gym.ObservationWrapper):
    ...
    def __init__(
        self, env, channel_dimensions=(observation_preprocessing.SMM_WIDTH, observation_preprocessing.SMM_HEIGHT)
    ):
        gym.ObservationWrapper.__init__(self, env)
        self._channel_dimensions = channel_dimensions
        action_shape = np.shape(self.env.action_space)
    ...
    def observation(self, obs):
        return observation_preprocessing.generate_smm(
            obs, channel_dimensions=self._channel_dimensions, config=self.env.unwrapped._config
        )
...
class SingleAgentObservationWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        gym.ObservationWrapper.__init__(self, env)
        self.observation_space = gym.spaces.Box(
            low=env.observation_space.low[0], high=env.observation_space.high[0], dtype=env.observation_space.dtype
        )
...
class MultiAgentToSingleAgent(gym.Wrapper):
    ...
    def get_action(actions, orginal_observation):
        assert "designated" in orginal_observation[0], "Only raw observations can be converted"
