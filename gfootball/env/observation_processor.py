...
class TextWriter(object):
    ...
    def __init__(self, frame, x, y=0, field_coords=False, color=(255, 255, 255)):
        ...
        self._lineType = 2
        self._arrow_types = ("top", "top_right", "right", "bottom_right", "bottom", "bottom_left", "left", "top_left")

    def write(self, text, scale_factor=1, color=None):
        textPos = (self._pos_x, self._pos_y)
        fontScale = 0.8 * scale_factor
        cv2.putText(self._frame, text, textPos, self._font, fontScale, color or self._color, self._lineType)
        ...
        for row in data:
            ...
            for col, cell in enumerate(row):
                ...
                if text in self._arrow_types:
                    ...
                    textPos = (self._pos_x, self._pos_y)
                    cv2.putText(self._frame, text, textPos, self._font, fontScale, color, self._lineType)
...
def get_frame(trace):
    ...
    writer = TextWriter(frame, trace["ball"][0], trace["ball"][1], field_coords=True, color=(248, 244, 236))
    writer.write("B")
    for player_idx, player_coord in enumerate(trace["left_team"]):
        writer = TextWriter(frame, player_coord[0], player_coord[1], field_coords=True, color=(238, 68, 47))
    ...
    for player_idx, player_coord in enumerate(trace["right_team"]):
        writer = TextWriter(frame, player_coord[0], player_coord[1], field_coords=True, color=(99, 172, 190))
        letter = str(player_idx)
...
class ActiveDump(object):
    ...
    def __init__(self, name, finish_step, config):
        ...
        if config["write_video"]:
            ...
            self._frame_dim = (config["render_resolution_x"], config["render_resolution_y"])
        ...
        if self._video_writer:
            ...
            if self._config["display_game_stats"]:
                ...
                if o["ball_owned_team"] == 0:
                    player = (
                        "G"
                        if o["left_team_roles"][o["ball_owned_player"]] == e_PlayerRole_GK
                    ...
                    player = (
                        "G"
                        if o["right_team_roles"][o["ball_owned_player"]] == e_PlayerRole_GK
                        else o["ball_owned_player"]
                    )
                ...
                for team in ["left", "right"]:
                    ...
                    for player in range(len(o[sticky_actions_field])):
                        ...
                        player_info = {}
                        player_info["color"] = (47, 68, 238) if team == "left" else (190, 172, 99)
                        player_info["id"] = (
                            "G" if o["%s_team_roles" % team][player_idx] == e_PlayerRole_GK else str(player_idx)
                        )
                        ...
                        for i in range(len(sticky_actions)):
                            if sticky_actions[i]._directional:
                                ...
                                player_info[sticky_actions[i]._name] = o[sticky_actions_field][player][i]

                        # Info about direction
                        player_info["DIRECTION"] = "O" if active_direction is None else active_direction._name
...
class ObservationProcessor(object):
    ...
    def write_dump(self, name):
        ...
        dump_name = "{2}{3}{0}_{1}".format(
            name, datetime.datetime.now().strftime("%Y%m%d-%H%M%S%f"), self._dump_directory, os.sep
        )
        config._active_dump = ActiveDump(dump_name, self._frame + config._steps_after, self._config)
    ...
    def process_pending_dumps(self, episode_done=False):
        ...
        for name in self._dump_config:
            config = self._dump_config[name]
            if config._active_dump and (episode_done or config._active_dump._finish_step <= self._frame):
