...
class FootballEnvTest(parameterized.TestCase):
    ...
    def test___control_all_players(self):
        ...
        env = gfootball.env.create_environment(
            env_name="tests.multiagent_wrapper", rewards="checkpoints,scoring", number_of_left_players_agent_controls=1
        )
        obs = env.reset()
        ...
        obs, _, _, _ = env.step([football_action_set.action_idle, football_action_set.action_idle])
        obs, _, _, _ = env.step([football_action_set.action_left, football_action_set.action_right])
        self.assertLen(obs, 2)
    ...
    def test_second_half(self):
        ...
        for _ in range(6):
            self.assertFalse(done)
            o, _, done, _ = env.step(football_action_set.action_idle)
            self.assertAlmostEqual(o[0]["left_team"][o[0]["active"]][0], -0.5, delta=0.1)
        self.assertTrue(done)
        env.close()
    ...
    def test_setstate(self, seed):
        ...
        cfg1 = config.Config(
            {"level": "tests.symmetric", "game_engine_random_seed": seed, "reverse_team_processing": False}
        )
        cfg2 = config.Config(
            {"level": "tests.symmetric", "game_engine_random_seed": seed + 10, "reverse_team_processing": False}
        )
    ...
    def test_render_state_equals_norender(self, seed):
        ...
        queue1 = Queue()
        thread1 = threading.Thread(target=run_scenario, args=(cfg1, queue1, actions, False, False))
        thread1.start()
        queue2 = Queue()
        thread2 = threading.Thread(target=run_scenario, args=(cfg2, queue2, actions, True, False))
        thread2.start()
        while True:
            o1 = queue1.get()
            o2 = queue2.get()
